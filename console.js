(async function () {
  ...
  if (!currentCustomerId) { console.error("❌ Giriş yapılmamış"); return; }

  // Eski widget + style temizle
  var old = document.getElementById("loyalty-widget");
  if (old) old.remove();
  document.querySelectorAll("style").forEach(function(s) {
    if (s.textContent.includes("loyalty-widget")) s.remove();
  });

  var style = document.createElement("style");
  style.textContent =
    "#loyalty-widget{position:fixed;bottom:20px;right:20px;z-index:9999;background:#fff;border:1px solid #e0e0e0;border-radius:12px;padding:14px 18px;box-shadow:0 2px 12px rgba(0,0,0,.12);font-family:inherit;font-size:14px;color:#333;min-width:180px}" +
    ...
    "#lw-redeem-btn{margin-top:10px;width:100%;padding:7px 0;background:#1a1a1a;color:#fff;border:none;border-radius:8px;font-size:12px;cursor:pointer}" +
    "#lw-redeem-btn:disabled{background:#aaa;cursor:default}" +
    "#lw-coupon-box{margin-top:8px;padding:6px 8px;background:#f5f5f5;border-radius:6px;font-size:13px;font-weight:700;letter-spacing:1px;text-align:center;display:none;cursor:pointer;color:#1a1a1a}" +
  ...
  widget.innerHTML =
    ...
    '<button id="lw-redeem-btn">Kupon Oluştur</button>' +
    '<div id="lw-coupon-box"></div>' +
    '<div id="lw-coupon-hint">Kopyalamak için tıkla</div>';
  document.body.appendChild(widget);

  fetch(BACKEND + "/loyalty/" + currentCustomerId)
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var balance = (data.wallet && data.wallet.points_balance) || 0;
      document.getElementById("lw-points").textContent = balance + " puan";
    });
